...
class AddTopicViewModel {
    ...
    func submitButtonTapped(title: String) {
        ...
        let dateOfCreation = dateFormatter.string(from: Date())

        dataManager.addTopic(title: title, raw: "Creating a new topic from the iOS APP ", createdAt: dateOfCreation) { [weak self] (result) in
            switch result {
